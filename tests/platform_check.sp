...
{ if ends_with(path, "/") { path } else { path + "/" } }
...
fn last_string(xs: List[String]) -> String { match xs {
    [x, ..rest] => { if len(rest) == 0 { x } else { last_string(rest) } },
    _ => "",
} }
...
{ if ends_with(dir, "/") { dir + file } else { dir + "/" + file } }
